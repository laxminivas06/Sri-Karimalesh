...
const categoryItems = {
  ...
  "Fry Items": [
    ...
    { name: "Potato Fry", singlePrice: 3.99, fullPrice: 7.99, img: "https://shwetainthekitchen.com/wp-content/uploads/2024/05/potato-fry.jpg" },
    ...
    { name: "Cabbage Fry", singlePrice: 4.99, fullPrice: 9.99, img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcStWy8tFttgZlYuptMjXogGsCmHMuacLpiFNg&s" },
  ],
  Starters: [
...
const CategoryPage: React.FC = () => {
  ...
  const [cart, setCart] = useState<{ name: string; price: number; img: string; pack: string; option?: string }[]>([]);
  ...
  const addToCart = (item: { name: string; singlePrice: number; fullPrice: number; img: string }, pack: string, option?: string) => {
    ...
    const updatedItem = {
      ...
      img: item.img,
      pack,
      option, // Add the selected option (e.g., "Dry", "Wet", "Single Roti", "Custom Choice")
    };
      ...
      {items.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {items.map((item, index) => {
            ...
            const [selectedOption, setSelectedOption] = useState(""); // Default option selection
